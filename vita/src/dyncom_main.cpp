...
constexpr const char* ProbeVersion = "02.00";
constexpr const char* LogPath = "ux0:data/azahar-vita/boot.log";
constexpr u32 BenchmarkIterations = 10'000;
...
static_assert(__cplusplus >= 202002L, "The Vita DynCom probe requires C++20");
static_assert(sizeof(void*) == 4, "The Vita DynCom probe must be built for a 32-bit target");
...
public:
    int Allocate() {
        uid = sceKernelAllocMemBlock("azahar-vita-dyncom-framebuffer",
...
int main() {
    ...
    Common::Log::Start();
    LOG_INFO(Core_ARM11, "dyncom_probe_version={} platform=vita cache_bytes={}", ProbeVersion,
             TRANS_CACHE_SIZE);
...
#if defined(AZAHAR_VITA_DYNCOM_FORCE_FAILURE)
    return Fail(&framebuffer, "forced_failure", static_cast<int>(0xA2000001U));
#endif

    const Vita::DyncomProbe::CorpusReport report = Vita::DyncomProbe::RunCorpus();
    for (const auto& group : report.groups) {
        if (group.passed) {
            LOG_INFO(Core_ARM11, "PASS group={} signature=0x{:08X} instructions={}", group.name,
                     group.signature, group.instructions);
        } else {
            LOG_ERROR(Core_ARM11, "FAIL group={} signature=0x{:08X} instructions={}", group.name,
                      group.signature, group.instructions);
        }
    }
    if (report.invalid_access) {
        return Fail(&framebuffer, "memory_bounds", -1);
    }
    LOG_INFO(Core_ARM11, "PASS memory bounds=clean");
    if (!report.Passed()) {
        return Fail(&framebuffer, "instruction_corpus", -1);
    }

    const u64 benchmark_start = sceKernelGetSystemTimeWide();
    const Vita::DyncomProbe::BenchmarkResult benchmark =
        Vita::DyncomProbe::RunBenchmark(BenchmarkIterations);
    const u64 benchmark_end = sceKernelGetSystemTimeWide();
    const u64 elapsed_us = benchmark_end - benchmark_start;
    if (!benchmark.passed || elapsed_us == 0) {
        return Fail(&framebuffer, "benchmark", -1);
    }
    const u64 instructions_per_second = benchmark.instructions * 1'000'000 / elapsed_us;
    LOG_INFO(Core_ARM11,
             "PASS benchmark iterations={} instructions={} elapsed_us={} instructions_per_second={}",
             BenchmarkIterations, benchmark.instructions, elapsed_us, instructions_per_second);
    ...
    LOG_INFO(Core_ARM11, "RESULT PASS instructions={}", report.instructions);
    Common::Log::Stop();
    return 0;
}
